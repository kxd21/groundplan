...
import { IconFile, IconFolder, IconPlus, IconSearch, IconTrash } from './icons.js';
            ...
            <div className="folder-tree-list">
              {folderRows.map(({ folder, depth, trail }) => (
                <button key={folder.id} className={scope === 'folder' && folderId === folder.id ? 'active' : ''} style={{ paddingLeft: 10 + depth * 14 }} onClick={() => chooseFolder(folder.id)} title={trail}>
                  ...
                  {folder.favorite && <small title="Favorite">★</small>}
                </button>
              ))}
              ...
              {displayedPlans.map((plan) => {
                ...
                return (
                  <article className={`${selected.has(keyFor(plan)) ? 'is-selected' : ''}${plan.missing ? ' is-missing' : ''}${currentPath === plan.path ? ' is-open' : ''}`} key={keyFor(plan)}>
                    ...
                    <button className={`folder-plan-star${plan.starred ? ' is-on' : ''}`} onClick={() => updatePlan(plan, { starred: !plan.starred })} title={plan.starred ? 'Unstar plan' : 'Star plan'} aria-label={plan.starred ? `Unstar ${plan.name}` : `Star ${plan.name}`}>★</button>
                  </article>
                );
              })}
          ...
          <aside className="folder-workspace-details">
            {selectedPlans.length ? (
              ...
              <>
                ...
                <button className="folder-favorite-toggle" onClick={() => void run(() => api.planFolderUpdate(selectedFolder.id, { favorite: !selectedFolder.favorite }), selectedFolder.favorite ? 'Removed from favorites' : 'Folder favorited')} disabled={working}>{selectedFolder.favorite ? '★ Favorite folder' : '☆ Add to favorites'}</button>
