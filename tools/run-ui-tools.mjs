#!/usr/bin/env node
/**
 * Launch Groundplan and run the plan-tools UI stress harness.
 *
...
const SAVE =
  ...
  path.join(process.env.HOME || '', 'Downloads', 'Groundplan-tools.rv4');
...
async function main() {
  ...
  try {
    ...
    await sleep(2000);
    console.log('\nRunning plan-tools UI stress…\n');
    const stress = spawn('node', ['tools/ui-stress-tools.mjs'], {
      cwd: ROOT,
