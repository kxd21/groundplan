...
const auditContrast = async (theme, surface) => {
  ...
  const expression = [
    ...
    '  /*',
    '   * Any CSS colour the browser understands, resolved through a canvas.',
    '   *',
    '   * The hand-written parser below reads rgb() and hex and nothing else, so',
    '   * a background serialised as color(srgb ...) — which is what color-mix()',
    '   * produces, and this stylesheet uses color-mix in several places — came',
    '   * back null and the node was SKIPPED. A skipped node is not a passing',
    '   * node: it is a contrast failure the gate cannot see. Letting the browser',
    '   * do the conversion means the gate keeps working whatever colour syntax',
    '   * the stylesheet moves to next.',
    '   */',
